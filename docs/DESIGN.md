...
| 2026-08-06 | Overlay gauges lose their plates | The translucent `#101010` card behind each gauge belongs to the editor idiom, not to footage — it boxes the instrument and hides the sailing. Gauges now sit straight on the frame, titled centred directly above the instrument, and every readout carries a 1 px `rgba(0,0,0,0.69)` outline so white digits survive bright water. Dark discs that *are* the instrument (compass card, heel trough) stay. Panels remain the rule inside the editor UI. |
